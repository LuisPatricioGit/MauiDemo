...
namespace MauiJogoForca
{
    public partial class MainPage : ContentPage, INotifyPropertyChanged
    {
        ...
        {
            var temp = resposta.Select(x => (palpite.IndexOf(x) >= 0 ? x : '_'))
                 .ToArray();
        ...
        private void TratarPalpite(char letra)
        {
            ...
            else if (resposta.IndexOf(letra) == -1) //Jogador falhou
            {
                erros++;
                AtualizaStatus();
                VerificaSePerdeu();
